'''
Encapsulates the game and round state information for the player
'''
...
class RoundState(namedtuple('_RoundState', ['button', 'street', 'pips', 'stacks', 'hands', 'deck', 'previous_state'])):
    ...
    def legal_actions(self):
        ...
        if self.street == 0 and self.button == 0:  # Small blind acting first in preflop
            return {CallAction, RaiseAction}
        if continue_cost == 0:
    ...
    def raise_bounds(self):
        ...
        min_contribution = min(max_contribution, continue_cost + max(continue_cost, BIG_BLIND)) + 1
    ...
    def proceed(self, action):
        ...
        if isinstance(action, CallAction):
            ...
            # both players acted
            contributions = new_pips[1-active] - new_pips[active]
            new_stacks[active] -= contributions
            new_pips[active] += contributions
